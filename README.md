...
mamba install -c conda-forge numpy pandas matplotlib seaborn scipy scikit-learn statsmodels -y
...
##### Input Data
  --matrix-dir: Matrix directory
  The script reads gene-centric matrices using the standard suffix naming (same as previous sections):
  *_tss_matrix.tsv
  *_gene_profile_matrix.tsv
  *_tes_matrix.tsv
  So your --matrix-dir typically contains files like:
  ...
  --fpkm: Expression table (TSV)
  The expression file must have:
  The first column as gene ID (index)
  Subsequent columns as expression values (FPKM/TPM/Counts)
  --expr-cols specifies which columns are used for each condition/group
  (0-based, counting columns after the gene ID index column). 
...
---
  ...
  --matrix-dir: Matrix directory (required)
  --in-group: Matrix prefixes to load (required; defines the multi-track layout)
  --in-names: Condition/group names aligned to --in-group (required)
  --in-ylabels: Track (mark) labels aligned to --in-group (required)
  --fpkm: Expression TSV file (required)
  --expr-cols: Expression columns for each condition/group (required)
  --cluster-region: Region used to build clustering features: TSS / gene / TES (required)
  --plot-regions: Regions to visualize (e.g., TSS,gene,TES)
  --k: Number of clusters (K in K-means) (required)
  --outdir / --out-prefix: Output directory and prefix (required)

---

##### How to specify multiple tracks (--in-group, --in-names, --in-ylabels)

--in-group defines what matrices to load and how to organize them into tracks:
  * Use ; to separate different marks/tracks (rows in the panel)
  * Use , to separate conditions/groups within the same mark (columns within that track)
--in-names must match the structure of --in-group (same number of ; blocks, and the same number of comma-separated items per block).
--in-ylabels provides one label per ; block (comma-separated).
