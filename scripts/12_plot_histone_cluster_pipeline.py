...
import matplotlib
import matplotlib as mpl  # for shared colorbar (legend)
matplotlib.use('Agg')
...
def _add_shared_colorbar(fig, cmap: str, vmin: float, vmax: float, label: str = 'Z-score',
                         right: float = 0.94, bottom: float = 0.12, width: float = 0.015, height: float = 0.76):
    """Add a single shared colorbar (legend) on the far right of the figure."""
    norm = mpl.colors.Normalize(vmin=vmin, vmax=vmax)
    sm = mpl.cm.ScalarMappable(norm=norm, cmap=cmap)
    sm.set_array([])
    cax = fig.add_axes([right, bottom, width, height])
...
def plot_panel_heatmaps_png(track_mats_z: List[Tuple[str, pd.DataFrame]],
    ...
    - clean    : no titles/ticks (easy editing; keeps cluster separators)
    ...
    labels_sorted = labels.loc[order]
    cluster_breaks = np.where(labels_sorted.values[1:] != labels_sorted.values[:-1])[0] + 1
    ...
    def _draw_one(fig, axes, annotated: bool):
        ...
        for i, (tname, mat) in enumerate(track_mats_z):
            ...
            # separators between K-means clusters (improves readability)
            if len(cluster_breaks) > 0:
                lw = 1.0 if annotated else 0.6
                for y in cluster_breaks:
                    ax.plot([0, region_spec.bins_total], [y, y], color='black', linewidth=lw, zorder=5)
    ...
    _add_shared_colorbar(fig, cmap=cmap, vmin=vmin, vmax=vmax, label='Z-score')
...
def plot_single_heatmaps_png(track_mats_z: List[Tuple[str, pd.DataFrame]],
    ...
    for tname, mat in track_mats_z:
        ...
        sns.heatmap(sub.values, cmap=cmap, yticklabels=False, xticklabels=False,
                    vmin=vmin, vmax=vmax, cbar=True, cbar_kws={'label': 'Z-score'}, ax=ax)
        # separators between K-means clusters
        if len(cluster_breaks) > 0:
            for y in cluster_breaks:
                ax.plot([0, region_spec.bins_total], [y, y], color='black', linewidth=1.0, zorder=5)
        ax.set_title(tname, fontsize=10)
        ax.set_xticks(region_spec.xticks_pos)
        ax.set_xticklabels(region_spec.xticks_labels, fontsize=8, rotation=0)
        for xp in [0, region_spec.bins_total]:
        ...
        if len(cluster_breaks) > 0:
            for y in cluster_breaks:
                ax.plot([0, region_spec.bins_total], [y, y], color='black', linewidth=0.6, zorder=5)
        ax.set_title('')
        ax.set_xlabel('')
        ax.set_ylabel('')
        ax.set_xticks([])
