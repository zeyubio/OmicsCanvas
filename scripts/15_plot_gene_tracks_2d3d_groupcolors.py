    ...
    for idx, (kind, payload) in enumerate(stack_items):
        ...
        if kind == "bam":
            ...
            if ylim_bam_global is not None:
                ...
                yl = None
            ax = plot_one_bam_track(fig, rect, cov, name, bam_colors_layers[gi][bi], x_visible=x_visible, ylim=yl)
            ax_list.append(ax)
        else:
    ...
    for tid, rect, gi, idx_in_group in positions:
        ...
        if kind == "bam":
            ...
            if ylim_bam_global is not None:
                ...
                yl = None
            ax = plot_one_bam_track(fig, rect, cov, nm, bam_colors_layers[gidx][ctxmode], x_visible=False, ylim=yl)
            ax_list.append(ax)
        else:
            if not meth_dir:
                raise ValueError("meth_dir is required when meth tracks are requested.")
