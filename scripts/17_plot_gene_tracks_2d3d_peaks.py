...
# -----------------------------
# Peak regions (MACS2 narrowPeak / BED)
# -----------------------------
_PEAK_PLACEHOLDER_ALIASES = {"", ".", "NA", "N/A", "NONE", "None", "null", "NULL", "-"}

def is_peak_placeholder(x: Optional[str], placeholder: str = ".") -> bool:
    """Return True if x means 'no peak file'.

    By default, we treat '.', 'NA', 'NONE', '-', or empty string as placeholders.
    Users can also set a custom placeholder via --peak-placeholder.
    """
    if x is None:
        return True
    x = str(x).strip()
    if x == "":
        return True
    if x == str(placeholder).strip():
        return True
    return x in _PEAK_PLACEHOLDER_ALIASES


def parse_layers_like(spec: Optional[str], template: List[List[str]], *, placeholder: str = ".") -> List[List[str]]:
    """Parse a layer spec but enforce the same ';' and ',' shape as template.

    - ';' separates groups
    - ',' separates tracks inside a group
    - Empty item is allowed and will be converted to placeholder (so you can write ',,' etc.)

    Notes:
      - If spec is an empty string (e.g. --peak-spec ''), it means all placeholders for all groups.
      - The number of ';' groups must match template (unless spec is empty).

    Examples (template has 1 group with 3 tracks):
      spec='a.narrowPeak,,c.narrowPeak' -> ['a.narrowPeak', '.', 'c.narrowPeak']
      spec='' or spec=' '              -> ['.', '.', '.']   (all placeholders)
    """
    if template is None:
        return []
    if spec is None:
    ...
    raw = str(spec)
    if raw.strip() == "":
        return [[str(placeholder)] * len(g) for g in template]

    groups = raw.split(";")
    if len(groups) != len(template):
        raise ValueError(
            f"--peak-spec must have the same number of ';' groups as --name-spec. {len(groups)} != {len(template)}"
        )

    out: List[List[str]] = []
    for gi, (g_raw, tpl) in enumerate(zip(groups, template)):
        n = len(tpl)
        g_raw = str(g_raw)
        if g_raw.strip() == "":
            out.append([str(placeholder)] * n)
            continue

        items_raw = g_raw.split(",")
        if len(items_raw) != n:
            raise ValueError(
                f"Group {gi}: --peak-spec items count != --name-spec items count. {len(items_raw)} != {n}. "
                "Tip: use empty placeholder like 'file1,,file3' or explicit '.' for missing peaks."
            )
        one: List[str] = []
        for it in items_raw:
            it = str(it).strip()
...
def merge_intervals(intervals: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
    """Merge overlapped/adjacent intervals (0-based half-open)."""
    if not intervals:
        return []
    intervals = sorted([(int(s), int(e)) for s, e in intervals if int(e) > int(s)], key=lambda x: (x[0], x[1]))
...
def peaks_in_window(
    ...
    *,
    ...
    """Read MACS2 peaks and return merged intervals in [window_start, window_end).

    We assume the peak file is BED-like (MACS2 narrowPeak/broadPeak):
      - col1: chrom
      - col2: start (0-based)
      - col3: end   (0-based, end-exclusive)

    Returned intervals are *relative* to window_start, i.e. [0, window_len).
    """
    ...
    with _open_text_maybe_gzip(peak_path) as fh:
        for line in fh:
            ...
            e2 = min(e, window_end)
            intervals.append((s2 - window_start, e2 - window_start))
...
def parse_color_map(spec: str) -> Dict[str, str]:
    """Parse a simple label->color mapping.

    Supported separators: ',' or ';'
    Supported key/value separators: ':' or '='

    Example:
      'G:#2ca02c,B:#1f77b4,R:#d62728'
    """
    if not spec:
        return {}
    out: Dict[str, str] = {}
    parts = re.split(r"[;,]\s*", str(spec).strip())
    for it in parts:
        if not it:
            continue
        if ":" in it:
            ...
            raise ValueError(f"Invalid --color-map entry: {it!r}. Expected NAME:COLOR")
        out[k.strip()] = v.strip()
    return out


def validate_layers_shape(a: List[List[str]], b: List[List[str]], a_name: str, b_name: str) -> None:
    if len(a) != len(b):
        raise ValueError(f"{a_name} and {b_name} must have the same number of ';' groups. {len(a)} != {len(b)}")
    for gi in range(len(a)):
        if len(a[gi]) != len(b[gi]):
            raise ValueError(
                f"Within group {gi}, {a_name} and {b_name} must have the same number of ',' items. "
                f"{len(a[gi])} != {len(b[gi])}"
            )


def assign_bam_colors(
    ...
    *,
    ...
    """Return per-track colors with the same ';' and ',' structure as name_layers.

    Priority:
      1) --color-spec (explicit colors per track; same structure as --name-spec)
      2) --color-map  (label->color mapping reused across groups; missing labels filled from cmap)
      3) --cmap       (colors assigned by unique label so repeated labels keep the same color)
    """
    # 1) explicit per-track colors
    if color_spec:
        c_layers = parse_layers(color_spec)
        validate_layers_shape(c_layers, name_layers, "--color-spec", "--name-spec")
        return c_layers

    # 2) mapping label -> color (reused across groups)
    mp = parse_color_map(color_map) if color_map else {}

    # Build a stable color palette for labels that are not in mp
    labels_order: List[str] = []
    for g in name_layers:
        for nm in g:
            if nm not in labels_order:
                labels_order.append(nm)

    # If there are labels without explicit colors, assign them from cmap consistently
    missing = [nm for nm in labels_order if nm not in mp]
    if missing:
        auto_cols = get_colors(cmap, len(missing))
        for nm, col in zip(missing, auto_cols):
            mp[nm] = col

    # Apply mapping to each track
    return [[mp[nm] for nm in g] for g in name_layers]
...
def plot_one_bam_track(
    ...
    *,
    ...
    li_po: int = 10,
    peak_intervals: Optional[List[Tuple[int, int]]] = None,
    ...
    # Peak shading (draw first so the coverage stays visible on top)
    if peak_intervals:
...
def plot_2d(
    ...
    *,
    ...
    # peak shading (MACS2 / BED; optional)
    ...
    bam_colors_layers = assign_bam_colors(name_layers, cmap=cmap, color_spec=color_spec, color_map=color_map)
    ...
    for gi, group in enumerate(name_layers):
        for bi, nm in enumerate(group):
            stack_items.append(("bam", (gi, nm, bi)))
    if meth_position != "before" and meth_groups:
        for gi, group in enumerate(meth_groups):
            for prefix in group:
                if meth_layout == "combined":
    ...
    for idx, (kind, payload) in enumerate(stack_items):
        ...
        if kind == "bam":
            ...
            if ylim_bam_global is not None:
                ...
                yl = None
            ax = plot_one_bam_track(
                fig, rect, cov, name, bam_colors_layers[gi][bi],
                x_visible=x_visible, ylim=yl,
                peak_intervals=(peak_intervals.get(gi, {}).get(name, []) if peak_intervals else None),
                peak_color=peak_color,
                peak_alpha=peak_alpha,
            )
            ax_list.append(ax)
        else:
            gi, name, ctxmode = payload
            if not meth_dir:
...
def plot_3d(
    ...
    *,
    ...
    # BAM colors (group-aware; can be overridden with --color-spec / --color-map)
    bam_colors_layers = assign_bam_colors(name_layers, cmap=cmap, color_spec=color_spec, color_map=color_map)
    ...
    for gi in range(max_groups):
        ...
        def add_meth():
            for pref in meth_prefixes:
                if meth_layout == "combined":
                    tid = f"meth:g{gi}:{pref}"
                    meta[tid] = ("meth", gi, pref, "combined")
                    tids.append(tid)
                else:
                    for ctx in ["CG", "CHG", "CHH"]:
                        tid = f"meth:g{gi}:{pref}:{ctx}"
                        meta[tid] = ("meth", gi, pref, ctx)
                        tids.append(tid)

        def add_bam():
            for bi, nm in enumerate(bam_names):
                tid = f"bam:g{gi}:{bi}:{nm}"
                meta[tid] = ("bam", gi, nm, bi)
                tids.append(tid)
    ...
    for tid, rect, gi, idx_in_group in positions:
        ...
        if kind == "bam":
            ...
            if ylim_bam_global is not None:
                ...
                yl = None
            ax = plot_one_bam_track(
                fig, rect, cov, nm, bam_colors_layers[gidx][ctxmode],
                x_visible=False, ylim=yl,
                peak_intervals=(peak_intervals.get(gidx, {}).get(nm, []) if peak_intervals else None),
                peak_color=peak_color,
                peak_alpha=peak_alpha,
            )
            ax_list.append(ax)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    # ---------- Peak shading (MACS2 narrowPeak / BED) ----------
    p.add_argument(
        "--peak-spec",
        default=None,
        help=(
            "Optional peak files (MACS2 narrowPeak / broadPeak / BED) for shading peak regions on BAM tracks.\n"
            "Syntax must match --name-spec (same ';' and ',' structure; one peak file per BAM track).\n"
            "You can use an explicit placeholder (default '.') for tracks without peaks,\n"
            "or leave an item empty (e.g. 'a.narrowPeak,,c.narrowPeak') and it will be treated as placeholder.\n\n"
            "Example (3 tracks):\n"
            "  --peak-spec 'A_peaks.narrowPeak,,C_peaks.narrowPeak'\n"
        ),
    )
    p.add_argument(
        "--peak-dir",
        default=".",
        help="Directory containing peak files (used if entries in --peak-spec are relative paths).",
    )
    p.add_argument(
        "--peak-placeholder",
        default=".",
        help="Placeholder token indicating 'no peak file' in --peak-spec (default: '.').",
    )
    p.add_argument(
        "--peak-color",
        default="black",
        help="Peak shading color (default: black; combined with --peak-alpha gives light gray effect).",
    )
    p.add_argument(
        "--peak-alpha",
        type=float,
        default=0.18,
        help="Peak shading alpha (0~1). Smaller = lighter, more transparent (default: 0.18).",
    )
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Manual colors for BAM tracks (same ';' and ',' structure as --name-spec). "
            "Overrides --cmap. Example: --color-spec '#2ca02c,#1f77b4,#d62728;#2ca02c,#1f77b4,#d62728'. "
            "Accepts hex (e.g. #RRGGBB) or Matplotlib color names."
        ),
    )
    p.add_argument(
        "--color-map",
        default=None,
        help=(
            "Reusable label->color mapping for BAM tracks. Format: 'LABEL:COLOR,LABEL:COLOR,...' "
            "(also supports '=' and ';'). Missing labels are filled from --cmap. "
            "Example: --color-map 'G:#2ca02c,B:#1f77b4,R:#d62728'."
        ),
    )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    # peak-spec must match the shape of name-spec (one peak file per BAM track)
    if args.peak_spec is not None:
        peak_layers = parse_layers_like(args.peak_spec, name_layers, placeholder=args.peak_placeholder)
        peak_dir = args.peak_dir
        for gi, group in enumerate(peak_layers):
            for pi, fp in enumerate(group):
                if is_peak_placeholder(fp, placeholder=args.peak_placeholder):
                    continue
                path = fp if os.path.isabs(fp) else os.path.join(peak_dir, fp)
                if not os.path.exists(path):
                    raise FileNotFoundError(
                        f"Peak file not found for group {gi}, track {pi}: {path}. "
                        "Use placeholder (e.g. '.') if this track has no peaks."
                    )
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    # Peaks: shade MACS2 peaks on each BAM track (optional)
    peak_intervals = None
    if args.peak_spec is not None:
        peak_layers = parse_layers_like(args.peak_spec, name_layers, placeholder=args.peak_placeholder)

        chrom = model["ch"]
        win_start = int(model["mrna_start"]) - int(model["distance"])
        win_end   = int(model["mrna_end"]) + int(model["distance"])

        peak_dir = args.peak_dir
        cache: Dict[str, List[Tuple[int, int]]] = {}
        peak_intervals = {gi: {} for gi in range(len(name_layers))}
        for gi, group in enumerate(name_layers):
            for label, peak_fp in zip(group, peak_layers[gi]):
                if is_peak_placeholder(peak_fp, placeholder=args.peak_placeholder):
                    peak_intervals[gi][label] = []
                    continue
                path = peak_fp if os.path.isabs(peak_fp) else os.path.join(peak_dir, peak_fp)
                if path not in cache:
                    cache[path] = peaks_in_window(path, chrom=chrom, window_start=win_start, window_end=win_end)
                peak_intervals[gi][label] = cache[path]
