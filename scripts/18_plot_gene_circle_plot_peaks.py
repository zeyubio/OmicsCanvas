...
# -*- coding: utf-8 -*-
"""
OmicsCanvas — Gene Circle Plot

This script draws a circular (ring) view of a single gene, with optional rings for BAM coverage and methylation.
...
def parse_group_ylims(spec: Optional[str], n_groups: int, arg_name: str = "group-ylims") -> Optional[List[Tuple[float, float]]]:
    """Parse per-group y-limits specification.

    spec:
        'auto' / None => return None (keep original auto scaling).
        Otherwise a ';'-separated list with length == n_groups.
        Each item can be either:
          - 'MAX'                (interpreted as 0,MAX)
          - 'MIN,MAX'            (explicit).
        Example (3 groups): '50;80;120' or '0,50;0,80;10,120'.
    """
    if spec is None:
        return None
    s = str(spec).strip()
    ...
    if len(parts) != int(n_groups):
        raise ValueError(f"{arg_name} expects {n_groups} group(s) but got {len(parts)}: {spec!r}")
    out: List[Tuple[float, float]] = []
    for p in parts:
        if "," in p:
            y0, y1 = _parse_minmax(p)
        else:
            y0, y1 = 0.0, float(p)
        if (not np.isfinite(y0)) or (not np.isfinite(y1)) or (y1 <= y0):
            raise ValueError(f"Invalid y-lim in {arg_name}: {p!r} (must satisfy max>min)")
        out.append((float(y0), float(y1)))
    return out
...
# NOTE: Deprecated (kept for backward reference, do not use)
def parse_group_ylims_v0(s: Optional[str], n_groups: int) -> Optional[List[Tuple[float, float]]]:
    """
...
# -----------------------------
# Peaks (MACS2 narrowPeak / broadPeak / BED)
# -----------------------------
def parse_layers_preserve(spec: Optional[str]) -> List[List[str]]:
    """
    Like parse_layers(), but preserves empty items between commas for alignment.
    ...
    for grp in s.split(";"):
        ...
        if grp == "":
            continue
        items = [x.strip() for x in grp.split(",")]  # keep empties
        layers.append(items)
    return layers


def _is_peak_placeholder(token: str, placeholder: str = ".") -> bool:
    t = "" if token is None else str(token).strip()
    if t == "":
        return True
    if t == placeholder:
        return True
...
def read_peak_table(path: str) -> pd.DataFrame:
    """
    Read a MACS2 peak file (narrowPeak/broadPeak/BED).
    Uses the first 3 columns: chrom, start, end.
    Supports: .gz (pandas auto), .zip (first non-directory member).
    """
    if not os.path.exists(path):
    ...
    if str(path).lower().endswith(".zip"):
        ...
        with zipfile.ZipFile(path, "r") as zf:
            ...
            if not members:
                raise ValueError(f"Peak zip is empty: {path}")
            # Prefer typical peak-like filenames
            preferred = [n for n in members if re.search(r"(narrowPeak|broadPeak|\.bed)$", n, re.IGNORECASE)]
            pick = preferred[0] if preferred else members[0]
    ...
    if df.shape[1] < 3:
        raise ValueError(f"Peak file must have >=3 columns (chrom,start,end): {path}")
    df = df.iloc[:, :3].copy()
    df.columns = ["ch", "st", "en"]
    ...
    df = df.dropna(subset=["st", "en"]).copy()
    df["st"] = df["st"].astype(int)
    df["en"] = df["en"].astype(int)
    return df


def peaks_in_window_rel(peaks: pd.DataFrame, ch: str, window_st: int, window_en: int) -> List[Tuple[int, int]]:
    """
    Return overlapped peak intervals (relative to window_st), half-open [st, en).
    """
    if peaks is None or peaks.empty:
        return []
    one = peaks[peaks["ch"].astype(str) == str(ch)].copy()
    if one.empty:
        return []
    one = one[(one["en"] > int(window_st)) & (one["st"] < int(window_en))].copy()
    if one.empty:
        return []
    one["st"] = one["st"].clip(lower=int(window_st), upper=int(window_en))
    one["en"] = one["en"].clip(lower=int(window_st), upper=int(window_en))
    one = one[one["en"] > one["st"]].copy()
    if one.empty:
        return []
    rel = list(zip((one["st"] - int(window_st)).astype(int).tolist(),
                   (one["en"] - int(window_st)).astype(int).tolist()))
    return rel


def circle_peak_polygons(intervals: List[Tuple[int, int]], r0: float, r1: float, one: float,
                         *, arc_points: int = 30) -> List[np.ndarray]:
    """
    Build annulus-sector polygons for each interval.
    Returns a list of (N,2) arrays suitable for matplotlib.patches.Polygon.
    """
    polys: List[np.ndarray] = []
    if not intervals:
        return polys

    n = max(int(arc_points), 6)
    for (st, en) in intervals:
        st = int(st)
        en = int(en)
        if en <= st:
            continue
        idx = np.linspace(st, en, n, endpoint=True)
        ang = -float(one) * idx
        co = np.cos(ang)
        si = np.sin(ang)

        outer = np.stack([co * float(r1), si * float(r1)], axis=1)
        inner = np.stack([co[::-1] * float(r0), si[::-1] * float(r0)], axis=1)
        poly = np.concatenate([outer, inner], axis=0)
        polys.append(poly)
    return polys


def draw_circle_peak_shading(ax: plt.Axes, intervals: List[Tuple[int, int]], *,
                             r0: float, r1: float, one: float,
                             color: str = "black", alpha: float = 0.18,
                             zorder: float = 2.0, arc_points: int = 30) -> None:
    """
    Draw semi-transparent shading for peak intervals on a ring.
    """
    if not intervals:
        return
    polys = circle_peak_polygons(intervals, r0, r1, one, arc_points=arc_points)
    for poly in polys:
        patch = Polygon(poly, closed=True, facecolor=color, edgecolor="none",
...
def draw_circle_track_frame(
    ...
    *,
    ...
    y_min: float = 0.0,
    y_max: float = 1.0,
    tick_angle_idx: int = 0,
    tick_len: float = 0.03,
    text_pad: float = 0.02,
    font_size: float = 8.0,
):
    """Draw an ipynb-style frame for a single ring (track): inner/outer arcs + start/end radial edges,
    and annotate y-limits (y_min and y_max) at the ring edge.
    """
    # 1) two concentric arcs (inner/outer)
    # use a moderate number of points for speed while keeping smooth.
    npts = max(int(window_len // 5), 800)
    idx = np.linspace(0, window_len, npts)
    ang = -one * idx
    c = np.cos(ang)
    s = np.sin(ang)
    ax.plot(c * r0, s * r0, color=frame_color, lw=lw, zorder=5)
    ax.plot(c * r1, s * r1, color=frame_color, lw=lw, zorder=5)

    # 2) start/end radial edges (as in user's notebook; the 1.02 factor in `one` keeps a small gap)
    for k in (0, window_len):
        th = -one * float(k)
        ck = math.cos(th)
        sk = math.sin(th)
        ax.plot([ck * r0, ck * r1], [sk * r0, sk * r1], color=frame_color, lw=lw, zorder=5)

    # 3) manual y-lim ticks (y_min and y_max) on the ring edge at a chosen angle
    k = int(tick_angle_idx)
    th = -one * float(k)
    ct = math.cos(th)
    st = math.sin(th)
    ...
    def _tick_at_radius(rr: float, label: str, va: str):
        # tick line
        x0 = ct * rr - tx * (tick_len / 2.0)
        y0 = st * rr - ty * (tick_len / 2.0)
        x1 = ct * rr + tx * (tick_len / 2.0)
        y1 = st * rr + ty * (tick_len / 2.0)
        ax.plot([x0, x1], [y0, y1], color=frame_color, lw=lw, zorder=6)
        # text slightly outside along tangent direction
        xt = ct * rr + tx * (tick_len / 2.0 + text_pad)
        yt = st * rr + ty * (tick_len / 2.0 + text_pad)
        ax.text(xt, yt, label, color=frame_color, fontsize=font_size, ha="center", va=va, zorder=6)

    # y_min at inner edge, y_max at outer edge
    ...
    if y_max >= 1000 or y_max < 0.01:
        ymax_lab = f"{y_max:.2g}"
    else:
        ymax_lab = f"{y_max:g}"

    _tick_at_radius(r0, ymin_lab, va="center")
    _tick_at_radius(r1, ymax_lab, va="center")
...
def plot_circle(
    ...
    *,
    ...
    bam_step: int = 10,
    bam_group_ylims: Optional[List[Tuple[float, float]]] = None,  # per ';' group y-lims for BAM rings
    # peaks (MACS2 peak shading; aligned to BAM rings)
    peak_intervals: Optional[Dict[int, Dict[str, List[Tuple[int, int]]]]] = None,
    peak_color: str = "black",
    peak_alpha: float = 0.18,
    peak_zorder: float = 2.0,
    peak_arc_points: int = 30,
    # ring geometry
    ...
    rings_meta: List[Dict[str, float]] = []  # each: {'r0','r1','ymin','ymax'}
    # 1) Methylation rings
    if meth_dir and meth_spec:
        ...
        for prefix in prefixes:
            ...
            if meth_layout == "combined":
                ...
                # record ring frame meta (one ring for combined contexts)
                rings_meta.append({'r0': float(ring_cursor), 'r1': float(ring_cursor + ring_width), 'ymin': 0.0, 'ymax': 1.0})
                ...
                for ctx in ["CHH", "CHG", "CG"]:
                    ...
                    lc.set_zorder(4)
                    ax.add_collection(lc)
                    # record ring frame meta (one ring per context in separate layout)
                    rings_meta.append({'r0': float(ring_cursor), 'r1': float(ring_cursor + ring_width), 'ymin': 0.0, 'ymax': 1.0})
                    max_r = max(max_r, ring_cursor + ring_width)
                    ring_cursor += ring_width + ring_gap
    ...
    if bam_cov is not None and name_layers is not None:
        ...
        for i, nm in enumerate(bam_names):
            ...
            # Peak shading (optional; aligned to BAM rings)
            if (peak_intervals is not None) and (gi_found is not None):
                intervals = peak_intervals.get(int(gi_found), {}).get(str(nm), [])
                if intervals:
                    draw_circle_peak_shading(
                        ax, intervals,
                        r0=float(ring_cursor), r1=float(ring_cursor + ring_width), one=float(one),
                        color=str(peak_color), alpha=float(peak_alpha),
                        zorder=float(peak_zorder), arc_points=int(peak_arc_points),
                    )

            # Determine y-limits for this ring.
            # If --bam-group-ylims is provided, all tracks within the same ';' group share the same ylim.
            if (bam_group_ylims is not None) and (gi_found is not None):
                y0, y1 = bam_group_ylims[gi_found]
                arr_plot = np.clip(arr.astype(float), float(y0), float(y1)) - float(y0)
                scale_max = float(y1 - y0)
                if (not np.isfinite(scale_max)) or scale_max <= 0:
                    scale_max = 1.0
                ymin_lab, ymax_lab = float(y0), float(y1)
            else:
                # auto: each ring uses its own max
                n = len(arr)
                idx = np.arange(0, n, max(int(bam_step), 1))
                vmax = float(np.nanmax(arr[idx])) if idx.size else float(np.nanmax(arr))
                if (not np.isfinite(vmax)) or vmax <= 0:
                    vmax = 1.0
                arr_plot = arr
                scale_max = float(vmax)
                ymin_lab, ymax_lab = 0.0, float(vmax)

            segs = circle_lines_from_array(arr_plot, ring_cursor, ring_width, one, step=bam_step, scale_max=scale_max)
            rings_meta.append({'r0': float(ring_cursor), 'r1': float(ring_cursor + ring_width), 'ymin': float(ymin_lab), 'ymax': float(ymax_lab)})
    ...
    # Hide the square axes frame; draw per-track (ring) frames in ipynb style when requested.
    for sp in ["top", "right", "left", "bottom"]:
        ax.spines[sp].set_visible(False)

    if keep_frame:
        for rm in rings_meta:
            draw_circle_track_frame(
                ax,
                r0=float(rm['r0']),
                r1=float(rm['r1']),
                one=float(one),
                ...
                lw=float(frame_lw),
                y_min=float(rm.get('ymin', 0.0)),
                y_max=float(rm.get('ymax', 1.0)),
                tick_angle_idx=0,
                tick_len=0.03,
                text_pad=0.02,
                font_size=8.0,
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="omicscanvas_gene_circle_plot.py",
        formatter_class=argparse.RawTextHelpFormatter,
        description=(
            "OmicsCanvas | Single-gene circle/ring plot\n\n"
            "This script draws a circular (ring) visualization for one gene, including:\n"
            "  - Gene structure ring (exons/CDS/UTR/intron direction)\n"
            "  - Optional methylation rings (CG/CHG/CHH; combined or separate)\n"
            "  - Optional BAM rings (spikes from coverage arrays)\n\n"
            "Layer syntax used by --bam-spec / --name-spec / --meth-spec:\n"
            "  - ';' separates groups\n"
            "  - ',' separates tracks within a group\n\n"
            "Example:\n"
            "  python omicscanvas_gene_circle_plot.py \\\n"
            "    --gff3 genome.gff3 --gene GeneX --distance 2000 \\\n"
            "    --bam-dir bam --bam-spec 'A.bam,B.bam;Input.bam' --name-spec 'H3K27me3,H3K36me3;Input' \\\n"
            "    --meth-dir CX_gene --meth-spec 'SRRxxx__GeneX' --circle-meth-layout combined \\\n"
            "    --out GeneX_circle.pdf\n"
        ),
    )

    p.add_argument("--out", required=True, help="Output figure path (.pdf/.png/.svg).")

    # gene model
    p.add_argument("--gff3", required=True, help="GFF3 annotation file.")
    p.add_argument("--gene", required=True, help="Target gene/transcript ID (mRNA ID in GFF3).")
    p.add_argument("--distance", type=int, default=1000, help="Up/downstream distance in bp used to define the window.")
    p.add_argument("--gene-keywords", default="ID", help="GFF3 attribute key for mRNA ID (default: ID).")
    p.add_argument("--exon-keywords", default="Parent", help="GFF3 attribute key for exon/CDS parent (default: Parent).")

    # BAM rings (optional but recommended)
    p.add_argument(
        "--bam-spec",
        default=None,
        help=(
            "Optional BAM files for rings, using ';' groups and ',' within group.\n"
            "If omitted, BAM rings are skipped."
        ),
    )
    p.add_argument(
        "--name-spec",
        default=None,
        help="Optional labels for BAM rings (same structure as --bam-spec).",
    )
    p.add_argument("--bam-dir", default=".", help="Directory containing BAM files (for relative paths in --bam-spec).")
    p.add_argument("--reads-length", type=int, default=150, help="Read length for coverage normalization.")
    p.add_argument("--seq-type", choices=["paired", "single"], default="paired", help="paired or single (for normalization).")
    p.add_argument("--circle-bam-step", type=int, default=10, help="Downsample step for BAM spikes (larger = faster/sparser).")
    p.add_argument(
        "--bam-group-ylims",
        default="auto",
        help=(
            "Set y-limits for BAM/RNA rings per ';' group (so tracks within a sample share the same ylim).\n"
            "Format: 'MAX1;MAX2;...' (interpreted as 0,MAX) or 'MIN,MAX;MIN,MAX;...'.\n"
            "The number of groups must match --bam-spec/--name-spec groups.\n"
            "Example (3 groups): --bam-group-ylims '50;50;200'\n"
            "Default: auto (each ring scales independently)."
        ),
    )

    # Peak shading (MACS2 peaks; optional)
    p.add_argument(
        "--peak-spec",
        default=None,
        help=(
            "Optional MACS2 peak files (narrowPeak/broadPeak/BED) aligned to BAM rings.\n"
            "Structure must match --bam-spec/--name-spec (same ';' groups and ',' tracks).\n"
            "Each item is a peak filename; use '.' (default placeholder) or empty to indicate no peaks for that ring.\n"
            "Example (3 rings): --peak-spec 'A_peaks.narrowPeak,B_peaks.narrowPeak,.'\n"
        ),
    )
    p.add_argument("--peak-dir", default=".", help="Directory containing peak files (for relative paths in --peak-spec).")
    p.add_argument("--peak-placeholder", default=".", help="Placeholder token meaning 'no peak' (default: '.').")
    p.add_argument("--peak-color", default="black", help="Peak shading color (default: black).")
    p.add_argument("--peak-alpha", type=float, default=0.18, help="Peak shading transparency (default: 0.18).")
    p.add_argument("--peak-arc-points", type=int, default=30, help="Polygon smoothness for peak shading (default: 30).")
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Methylation prefix spec. Each prefix loads: <prefix>__CG.tsv, <prefix>__CHG.tsv, <prefix>__CHH.tsv."
        ),
    )
    p.add_argument(
        "--circle-meth-layout",
        ...
        default="combined",
        help=(
            "Methylation ring layout:\n"
            "  combined: overlay CG/CHG/CHH on the same ring.\n"
            "  separate: allocate one ring per context."
        ),
    )
    p.add_argument("--meth-drop-me-zero", action="store_true", help="Drop rows with me==0 when reading methylation TSVs.")
    ...
    p.add_argument("--circle-ring-start", type=float, default=1.10, help="Starting radius for the first ring outside the gene.")
    p.add_argument("--circle-ring-width", type=float, default=0.20, help="Ring width (spike length scaling).")
    p.add_argument("--circle-ring-gap", type=float, default=0.08, help="Gap between rings.")

    # Frame & limits
    g = p.add_mutually_exclusive_group()
    g.add_argument("--circle-frame", dest="circle_frame", action="store_true", help="Draw a black frame box (default).")
    g.add_argument("--circle-no-frame", dest="circle_frame", action="store_false", help="Disable the frame box.")
    p.set_defaults(circle_frame=True)
    p.add_argument("--circle-frame-lw", type=float, default=1.0, help="Line width for the frame box.")
    p.add_argument("--circle-margin", type=float, default=0.15, help="Extra margin for auto-limits when --circle-lim is not set.")
    p.add_argument(
        "--circle-lim",
        type=float,
        default=None,
        help=(
            "Manual half-range for x/y limits. Example: --circle-lim 2.2 => xlim/ylim = [-2.2, 2.2].\n"
            "If not set, limits are auto-scaled from outermost ring + --circle-margin."
        ),
    )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.peak_spec and (not args.bam_spec):
        raise ValueError("--peak-spec requires --bam-spec/--name-spec (peaks are aligned to BAM rings).")

    if args.bam_spec:
        if not args.name_spec:
            raise ValueError("--name-spec is required when --bam-spec is provided.")
        ...
        for gi in range(len(bam_layers)):
            if len(bam_layers[gi]) != len(name_layers[gi]):
                raise ValueError(f"Group {gi}: BAM count != name count.")


        # Optional per-group y-lims validation
        _ = parse_group_ylims(args.bam_group_ylims, n_groups=len(bam_layers), arg_name='--bam-group-ylims')

        # Peak spec validation (must align to BAM rings)
        if args.peak_spec:
            peak_layers = parse_layers_preserve(args.peak_spec)
            if len(peak_layers) != len(name_layers):
                raise ValueError("--peak-spec must have the same number of ';' groups as --name-spec.")
            for gi in range(len(name_layers)):
                if len(peak_layers[gi]) != len(name_layers[gi]):
                    raise ValueError(
                        f"Group {gi}: peak-spec count != name-spec count. "
                        "Use '.' (or empty) as placeholder to keep alignment."
                    )

    if args.bam_spec and (pysam is None):
        raise RuntimeError("pysam is required to read BAM files. Install it via: conda install -c bioconda pysam")
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if args.bam_spec:
        ...
        bam_group_ylims = parse_group_ylims(args.bam_group_ylims, n_groups=len(bam_layers), arg_name='--bam-group-ylims')

    # Peaks (optional): compute per-ring peak intervals within the gene window
    peak_intervals = None
    if args.bam_spec and args.peak_spec:
        peak_layers = parse_layers_preserve(args.peak_spec)
        peak_intervals = {gi: {} for gi in range(len(name_layers))}
        ch = str(model['ch'])
        window_st = int(model['mrna_start']) - int(model['distance'])
        window_en = int(model['mrna_end']) + int(model['distance'])

        for gi in range(len(name_layers)):
            for peak_tok, lab in zip(peak_layers[gi], name_layers[gi]):
                if _is_peak_placeholder(peak_tok, args.peak_placeholder):
                    peak_intervals[gi][str(lab)] = []
                    continue
                pk_path = str(peak_tok)
                if (not os.path.isabs(pk_path)):
                    pk_path = os.path.join(str(args.peak_dir), pk_path)
                pk_df = read_peak_table(pk_path)
                peak_intervals[gi][str(lab)] = peaks_in_window_rel(pk_df, ch, window_st, window_en)
    ...
    plot_circle(
        ...
        meth_layout=args.circle_meth_layout,
        meth_drop_me_zero=args.meth_drop_me_zero,
        bam_cov=bam_cov,
        name_layers=name_layers,
        bam_step=args.circle_bam_step,
        bam_group_ylims=bam_group_ylims,
        peak_intervals=peak_intervals,
        peak_color=args.peak_color,
        peak_alpha=args.peak_alpha,
        peak_arc_points=args.peak_arc_points,
        ring_start=args.circle_ring_start,
        ring_width=args.circle_ring_width,
        ring_gap=args.circle_ring_gap,
        keep_frame=bool(args.circle_frame),
        frame_lw=float(args.circle_frame_lw),
        frame_color="black",
        margin=args.circle_margin,
        manual_lim=args.circle_lim,
        ylim_override=None,
    )
