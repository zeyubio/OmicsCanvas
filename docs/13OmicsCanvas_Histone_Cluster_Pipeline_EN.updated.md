...
> **Important note (treatments must be consistent across marks):**
> The pipeline assumes the **same treatment names and order** for all marks, and uses the first mark's
> treatment list as the global treatment axis. Keep each `;` block in `--in-names` identical (e.g. `G,B,R;G,B,R;...`).
...
```bash
python 13_plot_histone_cluster_pipline.py \
...
- `--include-marks` / `--exclude-marks` : filter marks by **mark name** (comma-separated, must match items in `--in-ylabels`).
  - `--include-marks` empty means “include all marks”.
  - Default `--exclude-marks` is `Input,Pol_II_Ser2P,Pol_II_Ser5P`.
...
- Z-score is computed **per gene across bins** (axis=1) for each track, then tracks are concatenated into the clustering feature matrix.
- `--k` : number of clusters.
- `--random-state` : for reproducibility.

### Heatmap rendering
- `--cmap` : colormap (e.g., `RdBu_r`).
- `--vmin`, `--vmax` : heatmap color scale range (default `-2` and `2`).
- Panel heatmap layout:
  - `--panel-cols`, `--panel-rows` (explicit grid)
  - or `--cols-per-row` (compat; used when `--panel-cols` is not set)
  - sizing: `--panel-cell-width`, `--panel-cell-height` (inches per panel)
  - or `--panel-figsize "W,H"` (overall inches; overrides cell size)
  - `--heatmap-dpi` (PNG DPI; panel + single-track)
...
- `figures/` : all plots
  - `<out-prefix>_<region>_panel_heatmaps_annot_k<k>.png` : annotated panel heatmap
  - `<out-prefix>_<region>_panel_heatmaps_clean_k<k>.png` : clean panel heatmap
  - `single_tracks/<region>/annot/*.png` and `single_tracks/<region>/clean/*.png` : per-track heatmaps (if enabled)
  - `*_cluster_profiles.pdf` : per-cluster mean profiles
  - `*_cluster_expr_boxplot.pdf` : expression distribution per cluster

- `<out-prefix>_cluster_region-<cluster-region>_k<k>_labels.tsv` : gene → cluster label
- `<out-prefix>_feature_matrix_<cluster-region>_zscore.tsv.gz` : concatenated zscore feature matrix used for clustering
...
- **Heatmaps look blank**: check `--vmin/--vmax` (or just keep defaults), and confirm matrices are not all zeros.
- **Panel PNG too big**: reduce `--panel-figsize` (or `--panel-cell-width/--panel-cell-height`) or increase `--panel-cols`.
