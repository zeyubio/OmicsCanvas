# 15_plot_methylation_profile_2d3d_english.py — Methylation Meta-profile Plot (2D/3D)

Repo script: `15_plot_methylation_profile_2d3d_english.py`
...
> If the file has multiple columns, the script uses the **first column** as the methylation value.
...
```bash
python 15_plot_methylation_profile_2d3d_english.py \
  --meth-dir CX_gene \
  --contexts CHH \
  --samples SRR9321764 \
  --labels SRR9321764 \
...
> **Useful defaults (from the script):** `--whole-suffix _whole_line.txt`, `--contexts CG,CHG,CHH`, `--distance 2000`, `--bins-st 50 --bins-body 100 --bins-en 50`, `--xpad 10`, `--cmap tab10`, `--fig-x 6 --fig-y 8`, `--edge 0.10 --x-offset 20 --y-offset 35`, `--mode both`.
...
- `--xpad`  
  Extra padding on both ends of the x-axis **in bins** (aesthetics only; default 10).
...
- `--standard-distance-labels`  
  Use the **conventional** distance labels: left `-distance` and right `+distance`.  
  By default, the script keeps the **legacy** style: left `+distance` and right `-distance`.

---

## 5) Troubleshooting

- **Mode=3d but console prints “Saved ...2D.pdf”**: the current script prints the 2D status line unconditionally; please rely on the actual output files in the working directory.
